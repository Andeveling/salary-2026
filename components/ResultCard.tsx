...
const ResultCard: React.FC<ResultCardProps> = ({
	...
	return (
		<div className="bg-white border-2 border-black shadow-hard flex flex-col h-full relative">
			{/* Header Badge style */}
			<div
				className={`${mainColor} border-b-2 border-black px-6 py-4 flex justify-between items-center ${textColor}`}
			>
			...
			<div className="p-6 grow space-y-6">
				...
				<div className="space-y-3 font-mono text-sm">
					{details.map((item) => (
						<div
							key={item.label}
							className="flex justify-between items-center group hover:bg-slate-100 p-1 transition-colors"
						>
							<span
								className={`${item.label.includes("Beneficios") ? "font-bold underline" : "font-bold text-gray-600"}`}
							>
								{item.label}
							</span>
							<span
								className={`font-bold ${
									item.isDeduction
										...
										: item.isBonus
											? "text-green-600"
											: item.label.includes("INCLUIDOS")
												? "bg-neon-green text-black px-1"
												: item.label.includes("COSTO")
